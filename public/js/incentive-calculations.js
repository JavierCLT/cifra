...
const IncentiveCalculator = {
    ...
    getProductionData: function(teamData, month) {
        ...
        bankingProducts.forEach(product => {
            const productKey = `product_${product.toLowerCase()}_productivity`;
            const abpaKey = `product_${product.toLowerCase()}_abpa`;
            
            // Get from the data structure (this needs to match your actual data)
            const productProductivity = teamData[productKey]?.[month] || 0;
            const productAbpa = teamData[abpaKey]?.[month] || 0;
            
            const accounts = Math.round(totalHeadcount * productProductivity * businessDays);
    ...
    calculateMetrics: function(teamId, month, productionData, compensableMetrics, qualityRatios) {
    ...
    getCompensableMetrics: async function(teamId) {
        try {
            const response = await fetch(`/api/incentives/compensable-metrics/${teamId}`);
